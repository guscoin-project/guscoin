...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Guscoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Guscoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Guscoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Guscoin 0.7 datadir."
    ;;
  3)
    echo "Detected Guscoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Guscoin 0.8 datadir."
    ;;
esac
